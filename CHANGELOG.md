...
- **`docs/audits/purview.md`** — fourteenth and final domain audit, completing the v3.4.0 audit umbrella. Resolves spike [#335](https://github.com/Galvnyz/CheckID/issues/335). Catalogs **27 unique patterns** across 5 sub-domains (DLP policy coverage, sensitivity labels / MIP, retention, eDiscovery + Insider Risk, anti-patterns). Maps against the 16 existing M365-scope Purview-related checks (`PURVIEW-*`, `COMPLIANCE-*`, plus cross-domain Power BI labels). Identifies **14 gap CheckIDs** + 4 low-priority cross-spike consolidations (sensitivity-label site protection with #337, Teams labels with #340, Copilot labels with #336), **3 narrative-refresh candidates**. Final audit closes out the v3.4.0 work; the `docs/audits/purview.md` doc includes a **completion summary** — all 14 spikes resolved, with 4 cross-cutting themes surfaced for v3.5 consideration (AZ-namespace boundary, namespace duplications, canonical-data-file pattern proposing 4 new reference files, detection-method surface diversity across 5 distinct contracts).
- **`docs/audits/power-platform.md`** — thirteenth domain audit. Resolves spike [#336](https://github.com/Galvnyz/CheckID/issues/336). Surfaces an inverse-of-typical coverage finding: **too much** duplicate Power BI coverage (26 checks across `PBI-*` + `POWERBI-*` with **11 confirmed duplicate pairs** — the consolidation chore originally flagged in #333) and **zero coverage** of the broader Power Platform (Power Apps, Power Automate, Dataverse, Copilot Studio, tenant isolation, environment hygiene, DLP). Catalogs **23 unique patterns** across 6 sub-domains (tenant isolation, environment hygiene, DLP policy coverage, sharing controls, Copilot governance, anti-patterns). Identifies **17 gap CheckIDs** plus the headline **namespace consolidation chore** (11 PBI/POWERBI duplicate pairs + 3 PBI-TENANT meta-checks). Proposes a fourth **canonical data file** (`data/power-platform-connectors.json` for connector classification reference) joining the pattern from `role-tiers.json`, `microsoft-first-party-appids.json`, and `transport-rule-actions.json`. First audit where detection lives in the Power Platform admin PowerShell module (`Microsoft.PowerApps.Administration.PowerShell`) rather than Microsoft Graph; appendix documents the auth flow distinction and 8 edge cases.
- **`docs/audits/defender-for-cloud-apps.md`** — twelfth domain audit. Resolves spike [#338](https://github.com/Galvnyz/CheckID/issues/338). Catalogs **24 canonical patterns** across 5 sub-domains (app connectors, OAuth app governance, Cloud Discovery, session policies, anomaly detection + alert policies). Surfaces a **stark coverage asymmetry**: only 1 MDCA-specific check (`DEFENDER-CLOUDAPPS-001` = "is MDCA enabled") versus 27 OAuth/app-governance checks in `ENTRA-ENTAPP-*` + `ENTRA-CONSENT-*` namespaces. The audit gap list focuses on uniquely-MDCA surfaces (Cloud Discovery, session policies, anomaly detection) rather than re-cataloguing OAuth governance which is already strong at the Entra layer. Identifies **15 gap CheckIDs**, **1 narrative-refresh candidate**, and **1 cross-spike consolidation** (MDCA session policy + CA App Control grant pairing — most common MDCA misconfiguration). First audit where detection lives in a per-tenant URL endpoint (`https://<tenant>.portal.cloudappsecurity.com/api/`) rather than Graph; appendix documents the auth flow distinction and 8 edge cases.
...
- **`ENTRA-SSPR-001` mapping mismatch corrected** ([#355](https://github.com/Galvnyz/CheckID/issues/355)). Despite the CheckID and prior `name` ("Ensure 'Self service password reset enabled' is set to 'All'") suggesting SSPR enablement (CIS M365 v6 §5.2.4.1), the underlying Graph signal that downstream collectors actually read is the **MFA Registration Campaign** (`/v1.0/policies/authenticationMethodsPolicy → registrationEnforcement.authenticationMethodsRegistrationCampaign`) — a separate authentication-methods feature that nudges weaker-method users to enroll Microsoft Authenticator. Microsoft historically nested the registration-campaign config under the SSPR policy object in Graph for legacy reasons, but functionally it's MFA enrollment acceleration, not password reset. Surfaced by [Galvnyz/M365-Assess#878](https://github.com/Galvnyz/M365-Assess/issues/878). Applied "Option C" from the issue: kept the `ENTRA-SSPR-001` CheckID for backwards compatibility (deferring rename to v1.0 cross-repo contract bump infrastructure), but updated `name` ("Ensure MFA Registration Campaign is enabled to nudge users toward stronger methods"), `category` (`SSPR` → `AUTHMETHOD`), `rationale`, `impact`, `references` (Microsoft Learn registration-campaign docs), and dropped the misleading CIS M365 v6 §5.2.4.1 mapping. The CIS 5.2.4.1 SSPR coverage is now an explicit gap that a future `ENTRA-SSPR-002` (genuine SSPR enablement check) will fill.
...
- **24 stale CIS M365 v6.0 recommendation IDs reconciled to v6.0.1** ([#352](https://github.com/Galvnyz/CheckID/issues/352)). After phase-1 enrichment landed, 24 of 180 CIS-mapped checks failed to pick up `sectionNumber` / `assessmentStatus` / `cisSafeguardsByVersion` / `references` because their `cisM365ControlId` referenced v6.0 numbering that CIS reorganized in v6.0.1. **11 renumbered** by title-match against the v6.0.1 crosswalk (e.g. `EXO-DKIM-001` 4.4 → 2.1.9, `INTUNE-SECURITY-001` 6.1 → 4.1, `PBI-TENANT-001` 8.1 → 9.1.8). **13 had their `cis-m365-v6` mapping removed** because v6.0.1 no longer covers the surface (no Forms-respond/collaborate controls, no PIM activation policy details, no audit-policy / Secure Score / Windows Update / SPO scripting / Teams guest-access controls — Intune-specific items moved to a separate Intune Benchmark and Power BI items consolidated in section 9). New Pester gate `#352 every cis-m365-v6.controlId resolves to a recommendation # in data/cis-m365-crosswalk.json` prevents regression.
